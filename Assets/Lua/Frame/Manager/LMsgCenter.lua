    ...
    --print ("managerIdmanagerIdmanagerId = "..managerId)
     ...
     --print ("managerId   msgcenter  ===="..managerId)
	  ...
	  elseif  managerId == LManagerID.LCharatorManager  then
         GloableConfig.HeartbeatTimeCount = 20 --20秒不发网络消息就给服务器发送心跳包
         if msg.state ~=nil and msg.state == 1 then
            --此时登录异常连接并且登录 
            error("      消息登录异常重新登录  state = 1 ，消息Id   managerId =   "..managerId)
               local tmpMsg = LMsgBase:New(LTCPEvent.Connect);
               --this:SendMsg(tmpMsg); 
                 LUIManager.GetInstance():SendMsg(tmpMsg);

                tmpMsg =  LMsgBase:New(LCharactorEvent.LUILogin)
                tmpMsg.ext1 = LLogin.ext1 
                --this:SendMsg(tmpMsg);
                LUIManager.GetInstance():SendMsg(tmpMsg);
        else
